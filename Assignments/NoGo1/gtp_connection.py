...
class GtpConnection():
    ...
    def __init__(self, go_engine,outfile = '/tmp/gtp_log', debug_mode = False):
        ...
        self.current_p = None; #initialize the current player

        self.commands = {
            "protocol_version": self.protocol_version_cmd,
            "quit": self.quit_cmd,
            "name": self.name_cmd,
            "boardsize": self.boardsize_cmd, #initialize the board size
            "showboard": self.showboard_cmd, #showboard
            "clear_board": self.clear_board_cmd, #clearboard
            "komi": self.komi_cmd,
    ...
    def play_cmd(self, args):
        ...
        try:
            ...
            for arg in args:
                arg_str += ' ' + arg
            self.respond("illegal move:%s wrong coordinate"%(arg_str))
            return
