...
class GoBoard(object):
    ...
    def solve(self, color=0):
        ...
        if win == None: # unable to find solution
            return None, None
    ...
    # Reference :https://webdocs.cs.ualberta.ca/~mmueller/courses/496-general/python/code/
    # initial call with full window
    def alphaBetaCall(self, colorInt):
        rootState = self.copy()
        rootState.to_play = colorInt
        depth = 1
        while True:
            Result, position = self.alphabetaDL(rootState, -10000, 10000, depth)
            if Result > 0:
    ...
    def staticallyEvaluateForToPlay(self):
        if self.get_winner() == self.to_play:
            return 1
        elif self.get_winner() == GoBoardUtil.opponent(self.to_play):
            return -1
        return 0
    #########################################################
